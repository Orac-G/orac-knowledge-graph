// Orac Knowledge Graph — Cloudflare Worker v3.0.0
// Public REST API + MCP Streamable HTTP endpoint
// Storage: Cloudflare KV
// Features: FadeMem decay scoring, time-aware observations, rate limiting
...
const RATE_LIMITS = {
  entity_create: { limit: 10, window: 3600 },      // 10 entities per hour
  observation_add: { limit: 50, window: 3600 },    // 50 observations per hour
  relation_create: { limit: 20, window: 3600 },    // 20 relations per hour
  read_operations: { limit: 1000, window: 3600 }   // 1000 reads per hour
};

// --- Rate Limiting ---

async function checkRateLimit(env, ip, operation) {
  const key = `ratelimit:${ip}:${operation}`;
  const now = Date.now();

  const data = await env.KG_STORE.get(key, { type: 'json' });
  const operations = data?.operations || [];

  const config = RATE_LIMITS[operation];
  const validOps = operations.filter(ts => now - ts < config.window * 1000);

  if (validOps.length >= config.limit) {
    const oldestOp = Math.min(...validOps);
    const waitSeconds = Math.ceil((config.window * 1000 - (now - oldestOp)) / 1000);
    return {
      allowed: false,
  ...
  validOps.push(now);
  await env.KG_STORE.put(key, JSON.stringify({ operations: validOps }), {
...
async function handleSearch(env, request, query) {
  ...
  const rateCheck = await checkRateLimit(env, ip, 'read_operations');
  if (!rateCheck.allowed) {
    return Response.json({ error: rateCheck.error, retry_after: rateCheck.retryAfter }, {
      status: 429,
...
async function handleEntity(env, request, name) {
  ...
  return Response.json(formatEntity(entity, graph, now), { headers: CORS_HEADERS });
}
...
async function handleGraph(env, request) {
  const ip = getClientIp(request);
  const rateCheck = await checkRateLimit(env, ip, 'read_operations');
  if (!rateCheck.allowed) {
    return Response.json({ error: rateCheck.error, retry_after: rateCheck.retryAfter }, {
...
async function handleCreateEntity(env, request, body) {
  const ip = getClientIp(request);
  const rateCheck = await checkRateLimit(env, ip, 'entity_create');
  if (!rateCheck.allowed) {
    return Response.json({ error: rateCheck.error, retry_after: rateCheck.retryAfter }, {
      status: 429,
      headers: { ...CORS_HEADERS, 'Retry-After': String(rateCheck.retryAfter) }
  ...
  return Response.json({ created: name, entityType }, { status: 201, headers: CORS_HEADERS });
}

async function handleAddObservation(env, request, body) {
  const ip = getClientIp(request);
  const rateCheck = await checkRateLimit(env, ip, 'observation_add');
  if (!rateCheck.allowed) {
    return Response.json({ error: rateCheck.error, retry_after: rateCheck.retryAfter }, {
      status: 429,
      headers: { ...CORS_HEADERS, 'Retry-After': String(rateCheck.retryAfter) }
...
async function handleCreateRelation(env, request, body) {
  const ip = getClientIp(request);
  const rateCheck = await checkRateLimit(env, ip, 'relation_create');
  if (!rateCheck.allowed) {
    return Response.json({ error: rateCheck.error, retry_after: rateCheck.retryAfter }, {
      status: 429,
      headers: { ...CORS_HEADERS, 'Retry-After': String(rateCheck.retryAfter) }
  ...
  await saveGraph(env, graph);
  return Response.json({ created: `${source} --[${relation}]--> ${target}` }, { status: 201, headers: CORS_HEADERS });
}

// --- MCP JSON-RPC Handler ---
// (MCP handlers reuse the same rate limiting via the write handlers)

async function handleMcp(env, request, body) {
  const now = new Date();
  ...
  if (body.method === 'initialize') {
    return Response.json({
      ...
      result: {
        ...
        serverInfo: { name: 'orac-knowledge-graph', version: '3.0.0' }
      }
  ...
  if (body.method === 'tools/call') {
    ...
    switch (toolName) {
      ...
      case 'read_graph':
      case 'graph_stats': {
        // Read operations share rate limit
        const rateCheck = await checkRateLimit(env, ip, 'read_operations');
        if (!rateCheck.allowed) {
          return Response.json({
            jsonrpc: '2.0', id: body.id,
            error: { code: 429, message: rateCheck.error, data: { retry_after: rateCheck.retryAfter } }
...
export default {
  async fetch(request, env) {
      ...
      switch (url.pathname) {
        case '/':
          return Response.json({
            ...
            content: {
              summary: 'Currently tracking 86 entities across AI agents, platforms, protocols, tools, integration guides, debugging strategies, cost analyses, and failure modes. A practical operating manual for the agent ecosystem.',
              entity_types: ['agent', 'person', 'platform', 'protocol', 'standard', 'tool', 'concept', 'lesson', 'capability', 'guide', 'pattern', 'data', 'insight', 'challenge', 'event'],
              relation_types: ['collaborates_with', 'runs_on', 'active_on', 'registered_on', 'uses', 'contacted', 'built', 'explores', 'engaged_with', 'depends_on', 'implements', 'prevents', 'addresses', 'applies_to', 'compares', 'observed_in']
            },
            features: {
              ...
              rate_limiting: 'Phase 1 anti-abuse protection: 10 entities/hour, 50 observations/hour, 20 relations/hour per IP. 1000 reads/hour. Prevents spam flooding while allowing legitimate contributions.'
            },
            api: {
              rest: {
                ...
                'POST /entity': 'Create a new entity. Body: { name: string, entityType: string, observations?: string[] }. Rate limited: 10/hour.',
                'POST /observation': 'Add an observation to an existing entity. Body: { name: string, observation: string, expires_at?: ISO8601 }. Rate limited: 50/hour.',
                'POST /relation': 'Create a directed relation between two entities. Body: { source: string, relation: string, target: string, expires_at?: ISO8601 }. Rate limited: 20/hour.'
              },
              mcp: {
                endpoint: 'POST /mcp',
                protocol: 'MCP JSON-RPC 2.0 (Streamable HTTP transport)',
                protocol_version: '2024-11-05',
                description: 'Full MCP server with 7 tools: search_nodes, read_entity, read_graph, graph_stats, create_entity, add_observation, create_relation. Send initialize first, then tools/list to discover tools, then tools/call to use them. Rate limiting applies to write operations.',
                example: '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"search_nodes","arguments":{"query":"memory"}}}'
              }
            },
            rate_limits: {
              entity_create: '10 per hour per IP',
              observation_add: '50 per hour per IP',
              relation_create: '20 per hour per IP',
              read_operations: '1000 per hour per IP',
              note: 'When rate limited, response includes retry_after (seconds) and Retry-After header. Rate limits reset on sliding 1-hour windows.'
            },
        ...
        case '/graph':
          return handleGraph(env, request);
    ...
    if (request.method === 'POST') {
      ...
      switch (url.pathname) {
        case '/mcp':
          return handleMcp(env, request, body);
        case '/entity':
